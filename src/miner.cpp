...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("HITEXMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("HITEXMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("HITEXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hitex-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("HITEXMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running HITEXMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadHITEXMiner() exception");
    } catch (...) {
        LogPrintf("ThreadHITEXMiner() exception");
    }

    LogPrintf("ThreadHITEXMiner exiting\n");
}
